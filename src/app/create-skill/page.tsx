    ...
    return (
      <div className="min-h-screen pt-24 pb-20 px-4 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold mb-2">智囊已创建！</h1>
          ...
          <div className="flex justify-center gap-3">
            <button
              ...
              className="px-5 py-2.5 bg-white text-black text-sm font-medium rounded-xl hover:bg-white/90 transition-all"
            >
              💡 去头脑风暴
            </button>
            <button
              onClick={() => router.push('/interrogate')}
              className="px-5 py-2.5 border border-white/20 text-white/60 text-sm rounded-xl hover:text-white hover:border-white/40 transition-all"
            >
              🔍 去质疑团
            </button>
            <button
    ...
    return (
      <div className="min-h-screen pt-24 pb-20 px-4 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/60 text-sm">正在分析资料，提炼思维框架…</p>
          <p className="text-white/30 text-xs mt-2">通常需要 10-20 秒</p>
        </div>
      </div>
    )
  }
      ...
      <div className="min-h-screen pt-24 pb-20 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="mb-8 text-center">
            <h1 className="text-2xl font-bold mb-1">预览与编辑</h1>
            <p className="text-white/40 text-sm">检查生成的思维框架，可直接编辑内容</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5 space-y-4">
                <h2 className="text-sm font-medium text-white/50 uppercase tracking-wider">智囊信息</h2>

                {/* Emoji picker */}
                <div>
                  <label className="text-xs text-white/40 mb-2 block">选择头像</label>
                ...
                <div>
                  <label className="text-xs text-white/40 mb-1.5 block">智囊名称 *</label>
                  <input
                    value={skillName}
                ...
                <div>
                  <label className="text-xs text-white/40 mb-1.5 block">简介</label>
                  <input
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 border border-white/20 text-white/60 text-sm rounded-xl hover:text-white hover:border-white/40 transition-all"
                >
                  重新生成
                </button>
                <button
                  onClick={saveSkill}
                  className="flex-1 py-2.5 bg-white text-black text-sm font-medium rounded-xl hover:bg-white/90 transition-all"
                >
                  保存智囊
                </button>
              </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              <h2 className="text-sm font-medium text-white/50 uppercase tracking-wider mb-3">思维框架内容</h2>
              <textarea
                value={editedContent}
                onChange={e => setEditedContent(e.target.value)}
                rows={20}
                className="w-full bg-transparent text-xs text-white/70 font-mono resize-none focus:outline-none leading-relaxed"
              />
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen pt-24 pb-20 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="mb-10 text-center">
          ...
          <h1 className="text-3xl font-bold mb-2">创建智囊</h1>
        ...
        <div className="space-y-5">
          {/* Person name */}
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5 space-y-4">
            <h2 className="text-sm font-medium text-white/50 uppercase tracking-wider">人物信息</h2>
            <input
              value={personName}
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5 space-y-4">
            <div className="flex gap-1 p-1 bg-white/[0.03] rounded-xl w-fit">
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm transition-all ${
                  inputMode === 'text' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/60'
                }`}
              >
                粘贴文本
              </button>
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm transition-all ${
                  inputMode === 'url' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white/60'
                }`}
              >
                网页链接
              </button>
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white/[0.04] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-white/25 transition-colors"
                  />
                  <button
                    onClick={fetchUrl}
                    disabled={urlLoading || !urlInput.trim()}
                    className="px-4 py-2.5 bg-white/10 text-white text-sm rounded-xl hover:bg-white/15 transition-all disabled:opacity-30 disabled:cursor-not-allowed whitespace-nowrap"
                  >
                    {urlLoading ? '抓取中…' : '抓取'}
                  </button>
                </div>
                {urlError && <p className="text-red-400 text-xs">{urlError}</p>}
                <p className="text-xs text-white/20">仅支持公开静态页面，不支持需要登录的内容</p>
              </div>
            )}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-white/20">{rawText.length} / 5000 字</span>
              </div>
              <textarea
                value={rawText}
